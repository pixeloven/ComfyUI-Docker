...
2. **Change the `uvx` install line.** The old
   `uvx --from git+https://github.com/pixeloven/ComfyUI-Docker#subdirectory=services/fetch comfyfetch …`
   becomes
   `uvx --from git+https://github.com/pixeloven/ComfyUI-Docker#subdirectory=services/comfyctl comfyctl fetch …`.
   Pin it with `@v4.0.0` after the repository URL.
3. **Install the release wheels as a pair.** A release now ships
   ...
   `comfyctl` pins exactly. Neither is on PyPI, so name both:
...
4. **The fetch image's entrypoint is now `comfyctl fetch fetch`**, where it was
   `comfyfetch fetch`. Arguments appended by `docker run`, a Compose `command:`
   or a Kubernetes Job's `args:` (such as `/lock.yaml /app --apply`) mean what
   they did before, so the example Compose files need no change. **An override
   that names the old binary breaks.** Change `command: ["comfyfetch"]` to
   `command: ["comfyctl", "fetch"]`, and `--entrypoint comfyfetch` to
   `--entrypoint comfyctl` with `fetch` as the first argument.
5. **Contributors:** `services/` is now a uv workspace, with `services/fetch`
   and `services/comfyctl` as members and one lock at `services/uv.lock` (moved
   from `services/fetch/uv.lock`). Run `cd services && uv run pytest -q`.
...
- **`comfyctl`**, the umbrella CLI (`services/comfyctl`). Every group shares one
  ...
  request). A test asserts every command takes the output flag. `comfyctl relay`
  follows in [#131](https://github.com/pixeloven/ComfyUI-Docker/issues/131).
- **CI checks more version files on every push**:
  `services/comfyctl/pyproject.toml`, its `comfyfetch==` pin, and both
  workspace entries in `services/uv.lock`. uv doesn't hold a workspace member
  to that pin, so without the check a partial bump would lock cleanly and ship
  two wheels that refuse to install together.
