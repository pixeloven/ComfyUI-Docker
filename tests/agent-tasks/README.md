...
|---|---|---|
| [Comfy-Org/comfy-mcp](https://github.com/Comfy-Org/comfy-mcp) | stdio, `uv run` with `comfy-mcp==0.10.0`, `comfy-cli==1.21.0` and `mcp==2.2.0` | `servers/comfy-mcp.json` |
| [artokun/comfyui-mcp](https://github.com/artokun/comfyui-mcp) | streamable HTTP on `127.0.0.1:9100/mcp` with a bearer token, `npx comfyui-mcp@0.52.203` | `servers/artokun.json` and `artokun.sh` |
...
HARNESS_RUN=1 ./run-all.sh   # the real evaluation, 3 servers x 4 tasks. Billed; needs the owner's approval.
...
- **Other knobs.** `HARNESS_TIMEOUT` (default 900 s), `HARNESS_BUDGET_USD` (default 5 per run), `HARNESS_MODEL`, and `HARNESS_BARE=1`. `HARNESS_BARE=1` adds `--bare`, which also skips CLAUDE.md and user memory, but it needs `ANTHROPIC_API_KEY`.
...
| T4 Failure | Submit `T4-workflow.json` unchanged, and report what happened in `results/T4.json`. | `succeeded` is `false`, `node_id` names the node ComfyUI rejected, and the report quotes ComfyUI's per-node error type or message. Setup captures that error from a real `/prompt` call. |
...
`run.sh` appends one row per run to `results/scorecard.csv`, with these columns: `timestamp, server, task, result, detail, seconds, mode, turns, cost_usd`. `mode` is `agent` or `dry-run`, and only `agent` rows count. The transcript goes to `results/runs/<server>-<task>.json`, and whatever the agent wrote goes beside it.
...
## Findings from building the harness (no agent runs yet)
...
2. **comfy-mcp's tool errors are masked on the current MCP SDK.** It resolves `mcp` 2.2.0, which reports any non-`ToolError` exception as just `Error executing tool <name>`. With `mcp==2.0.0` the real message comes through. comfy-mcp 0.10.0 was released before 2.1 and 2.2, and main hasn't changed this. The config pins 2.2.0, which is what a fresh install gets; `COMFY_MCP_SDK_VERSION=2.0.0` switches it.
3. **comfy-mcp's `run_workflow` validates on the client side and never reaches `/prompt`.** So an agent sees comfy-cli's wording (`input 'images' expects IMAGE but LoadImage[1] produces MASK`), not ComfyUI's `return_type_mismatch`. T4 as specified fails that. The check marks it as naming the same fault in other words, and the owner decides whether that counts.
4. **artokun's `safe` preset withholds tools the tasks need.** It drops `create_workflow` (node info and validation), `install_custom_node`, `upload_image` and `queue`, leaving 16 tools where the full surface has 41. Under `safe`, T2 can't pass and T3 has no introspection tool. The default here is `safe`; set `ARTOKUN_TOOL_PRESET=` (empty) for the full surface.
